...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
      ...
      <div class="flex">
        ...
        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
          ...
          @if (auth()->user()->role == 'pengunjung')
            <x-nav-link :href="route('badge')" :active="request()->routeIs('badge')" wire:navigate>
              {{ __('Lencana') }}
            </x-nav-link>
          @endif
          @if (auth()->user()->role == 'admin')
            <x-nav-link :href="route('manage-badge')" :active="request()->routeIs('manage-badge')" wire:navigate>
              {{ __('Kelola Lencana') }}
            </x-nav-link>
          @endif
  ...
  <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
    <div class="pt-2 pb-3 space-y-1">
      ...
      @if (auth()->user()->role == 'pengunjung')
        <x-responsive-nav-link :href="route('badge')" :active="request()->routeIs('badge')" wire:navigate>
          {{ __('Lencana') }}
        </x-responsive-nav-link>
      @endif
      @if (auth()->user()->role == 'admin')
